[Ivy]
17945B68D64A9057 9.2.0 #module
>Proto >Proto Collection #zClass
Dt0 DemoStart Big #zClass
Dt0 B #cInfo
Dt0 #process
Bk1 BpmnScriptTask Big #zClass
Bk1 BpmnScriptTask #cInfo
Bk2 BpmnServiceTask Big #zClass
Bk2 BpmnServiceTask #cInfo
Bk3 BpmnScriptTask Big #zClass
Bk3 BpmnScriptTask #cInfo
Bk4 BpmnServiceTask Big #zClass
Bk4 BpmnServiceTask #cInfo
Bk5 BpmnServiceTask Big #zClass
Bk5 BpmnServiceTask #cInfo
Bk0 BpmnScriptTask Big #zClass
Bk0 BpmnScriptTask #cInfo
Bk6 BpmnScriptTask Big #zClass
Bk6 BpmnScriptTask #cInfo
Dt0 @AnnotationInP-0n ai ai #zField
Dt0 @TextInP .type .type #zField
Dt0 @TextInP .processKind .processKind #zField
Dt0 @TextInP .xml .xml #zField
Dt0 @TextInP .responsibility .responsibility #zField
Dt0 @EndTask f11 '' #zField
Dt0 @InfoButton f18 '' #zField
Dt0 @StartRequest f15 '' #zField
Dt0 @UserTask f1 '' #zField
Dt0 @TkArc f2 '' #zField
Dt0 @PushWFArc f5 '' #zField
Dt0 @EndTask f10 '' #zField
Dt0 @StartRequest f4 '' #zField
Dt0 @StartRequest f16 '' #zField
Dt0 @StartRequest f17 '' #zField
Dt0 @EndTask f19 '' #zField
Dt0 @EndTask f20 '' #zField
Dt0 @PushWFArc f23 '' #zField
Dt0 Bk1 S11 'Sub 1' #zField
Dt0 @PushWFArc f3 '' #zField
Dt0 Bk2 S21 'Sub 2' #zField
Dt0 @PushWFArc f6 '' #zField
Dt0 @PushWFArc f8 '' #zField
Dt0 @PushWFArc f12 '' #zField
Dt0 Bk3 S31 'Sub 3' #zField
Dt0 Bk4 S41 'Sub 4' #zField
Dt0 @PushWFArc f0 '' #zField
Dt0 Bk5 S51 'Sub 5' #zField
Dt0 Bk0 S60 'Sub 6' #zField
Dt0 @PushWFArc f7 '' #zField
Dt0 @PushWFArc f13 '' #zField
Dt0 Bk6 S61 'Sub 6' #zField
Dt0 @PushWFArc f14 '' #zField
Dt0 @PushWFArc f9 '' #zField
Dt0 @InfoButton f21 '' #zField
Dt0 @InfoButton f22 '' #zField
Dt0 @InfoButton f24 '' #zField
Dt0 @InfoButton f25 '' #zField
>Proto Dt0 Dt0 DemoStart #zField
Bk1 @AnnotationInP-0n ai ai #zField
Bk1 @TextInP .type .type #zField
Bk1 @TextInP .processKind .processKind #zField
Bk1 @TextInP .xml .xml #zField
Bk1 @TextInP .responsibility .responsibility #zField
Bk1 @GridStep f89 '' #zField
Bk1 @PushTrueWFInG-01 g0 '' #zField
Bk1 @PushWFArc f0 '' #zField
Bk1 @PushTrueWFOutG-01 g1 '' #zField
Bk1 @PushWFArc f1 '' #zField
>Proto Bk1 Bk0 BpmnScriptTask #zField
Bk2 @AnnotationInP-0n ai ai #zField
Bk2 @TextInP .type .type #zField
Bk2 @TextInP .processKind .processKind #zField
Bk2 @TextInP .xml .xml #zField
Bk2 @TextInP .responsibility .responsibility #zField
Bk2 @CallSub f6 '' #zField
Bk2 @PushTrueWFInG-01 g0 '' #zField
Bk2 @PushWFArc f0 '' #zField
Bk2 @PushTrueWFOutG-01 g1 '' #zField
Bk2 @PushWFArc f1 '' #zField
>Proto Bk2 Bk1 BpmnServiceTask #zField
Bk3 @AnnotationInP-0n ai ai #zField
Bk3 @TextInP .type .type #zField
Bk3 @TextInP .processKind .processKind #zField
Bk3 @TextInP .xml .xml #zField
Bk3 @TextInP .responsibility .responsibility #zField
Bk3 @GridStep f100 '' #zField
Bk3 @PushTrueWFInG-01 g0 '' #zField
Bk3 @PushWFArc f0 '' #zField
Bk3 @PushTrueWFOutG-01 g1 '' #zField
Bk3 @PushWFArc f1 '' #zField
>Proto Bk3 Bk2 BpmnScriptTask #zField
Bk4 @AnnotationInP-0n ai ai #zField
Bk4 @TextInP .type .type #zField
Bk4 @TextInP .processKind .processKind #zField
Bk4 @TextInP .xml .xml #zField
Bk4 @TextInP .responsibility .responsibility #zField
Bk4 @CallSub f0 '' #zField
Bk4 @PushTrueWFInG-01 g0 '' #zField
Bk4 @PushWFArc f1 '' #zField
Bk4 @PushTrueWFOutG-01 g1 '' #zField
Bk4 @PushWFArc f2 '' #zField
>Proto Bk4 Bk3 BpmnServiceTask #zField
Bk5 @AnnotationInP-0n ai ai #zField
Bk5 @TextInP .type .type #zField
Bk5 @TextInP .processKind .processKind #zField
Bk5 @TextInP .xml .xml #zField
Bk5 @TextInP .responsibility .responsibility #zField
Bk5 @CallSub f21 '' #zField
Bk5 @PushTrueWFInG-01 g0 '' #zField
Bk5 @PushWFArc f0 '' #zField
Bk5 @PushTrueWFOutG-01 g1 '' #zField
Bk5 @PushWFArc f1 '' #zField
>Proto Bk5 Bk4 BpmnServiceTask #zField
Bk0 @AnnotationInP-0n ai ai #zField
Bk0 @TextInP tags tags #zField
Bk0 @MessageFlowInP-0n messageIn messageIn #zField
Bk0 @MessageFlowOutP-0n messageOut messageOut #zField
Bk0 @TextInP .xml .xml #zField
Bk0 @TextInP .responsibility .responsibility #zField
Bk0 @PushTrueWFInG-01 g0 '' #zField
Bk0 @PushTrueWFOutG-01 g1 '' #zField
Bk0 @GridStep f1 '' #zField
Bk0 @PushWFArc f2 '' #zField
Bk0 @PushWFArc f0 '' #zField
>Proto Bk0 Bk5 BpmnScriptTask #zField
Bk6 @AnnotationInP-0n ai ai #zField
Bk6 @TextInP tags tags #zField
Bk6 @MessageFlowInP-0n messageIn messageIn #zField
Bk6 @MessageFlowOutP-0n messageOut messageOut #zField
Bk6 @TextInP .xml .xml #zField
Bk6 @TextInP .responsibility .responsibility #zField
Bk6 @PushTrueWFInG-01 g0 '' #zField
Bk6 @PushTrueWFOutG-01 g1 '' #zField
Bk6 @GridStep f1 '' #zField
Bk6 @PushWFArc f2 '' #zField
Bk6 @PushWFArc f0 '' #zField
>Proto Bk6 Bk6 BpmnScriptTask #zField
Dt0 f11 617 201 30 30 0 15 #rect
Dt0 f18 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>Sign your docs by A-Trust&#13;
Your process opens a dialog.&#13;
This dialog allows the user uploads a document.&#13;
Then he can press sign the doc. Integration code will be call, and user will be redirect to A-Trust login page&#13;
</name>
        <nameStyle>25,5,8,0
193,5
</nameStyle>
    </language>
</elementInfo>
' #txt
Dt0 f18 128 82 592 92 -290 -42 #rect
Dt0 f15 outLink StartATrustExample.ivp #txt
Dt0 f15 inParamDecl '<> param;' #txt
Dt0 f15 requestEnabled true #txt
Dt0 f15 triggerEnabled false #txt
Dt0 f15 callSignature StartATrustExample() #txt
Dt0 f15 persist false #txt
Dt0 f15 startName 'A-Trust HandySignatur Example' #txt
Dt0 f15 caseData 'businessCase.attach=true
case.name=Your process' #txt
Dt0 f15 showInStartList 1 #txt
Dt0 f15 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>StartATrustExample.ivp</name>
    </language>
</elementInfo>
' #txt
Dt0 f15 @C|.responsibility Everybody #txt
Dt0 f15 137 201 30 30 -24 17 #rect
Dt0 f1 dialogId atrust.demo.DocumentSignature #txt
Dt0 f1 startMethod start() #txt
Dt0 f1 requestActionDecl '<> param;' #txt
Dt0 f1 responseMappingAction 'out=in;
' #txt
Dt0 f1 caseData 'case.name=A-Trust Demo' #txt
Dt0 f1 taskData 'TaskA.NAM=Start A-Trust Demo
TaskA.SKIP_TASK_LIST=true' #txt
Dt0 f1 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>DocumentSignature</name>
    </language>
</elementInfo>
' #txt
Dt0 f1 272 194 128 44 -55 -8 #rect
Dt0 f2 167 216 272 216 #arcP
Dt0 f5 400 216 617 216 #arcP
Dt0 f10 1009 393 30 30 0 15 #rect
Dt0 f4 outLink AddTemplateATrust.ivp #txt
Dt0 f4 inParamDecl '<> param;' #txt
Dt0 f4 requestEnabled true #txt
Dt0 f4 triggerEnabled false #txt
Dt0 f4 callSignature AddTemplateATrust() #txt
Dt0 f4 caseData businessCase.attach=true #txt
Dt0 f4 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>AddTemplateATrust.ivp</name>
    </language>
</elementInfo>
' #txt
Dt0 f4 @C|.responsibility Everybody #txt
Dt0 f4 137 393 30 30 -21 17 #rect
Dt0 f16 outLink DeleteTemplateATrust.ivp #txt
Dt0 f16 inParamDecl '<> param;' #txt
Dt0 f16 requestEnabled true #txt
Dt0 f16 triggerEnabled false #txt
Dt0 f16 callSignature DeleteTemplateATrust() #txt
Dt0 f16 caseData businessCase.attach=true #txt
Dt0 f16 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>DeleteTemplateATrust.ivp</name>
    </language>
</elementInfo>
' #txt
Dt0 f16 @C|.responsibility Everybody #txt
Dt0 f16 137 593 30 30 -21 17 #rect
Dt0 f17 outLink GetListTemplate.ivp #txt
Dt0 f17 inParamDecl '<> param;' #txt
Dt0 f17 requestEnabled true #txt
Dt0 f17 triggerEnabled false #txt
Dt0 f17 callSignature GetListTemplate() #txt
Dt0 f17 caseData businessCase.attach=true #txt
Dt0 f17 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>GetListTemplate.ivp</name>
    </language>
</elementInfo>
' #txt
Dt0 f17 @C|.responsibility Everybody #txt
Dt0 f17 137 817 30 30 -24 17 #rect
Dt0 f19 785 593 30 30 0 15 #rect
Dt0 f20 785 817 30 30 0 15 #rect
Dt0 f23 167 408 272 408 #arcP
Dt0 S11 .xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language lang="en">
        <name>Get TemplateSiegel from CMS&#13;
/Template/templatesiegel</name>
    </language>
</elementInfo>
' #txt
Dt0 S11 272 386 192 44 -77 -16 #rect
Dt0 f3 464 408 512 408 #arcP
Dt0 S21 .xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language lang="en">
        <name>Call ATrustSigner/TemplateManagement&#13;
AddTemplateATrust</name>
    </language>
</elementInfo>
' #txt
Dt0 S21 512 386 256 44 -107 -16 #rect
Dt0 f6 167 608 272 608 #arcP
Dt0 f8 400 608 480 608 #arcP
Dt0 f12 720 608 785 608 #arcP
Dt0 S31 .xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language lang="en">
        <name>Prepare Template id &#13;
which will be deleted</name>
    </language>
</elementInfo>
' #txt
Dt0 S31 272 586 128 44 -57 -20 #rect
Dt0 S41 .xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language lang="en">
        <name>Call ATrustSigner/TemplateManagement&#13;
DeleteTemplateATrust</name>
    </language>
</elementInfo>
' #txt
Dt0 S41 480 586 240 44 -111 -20 #rect
Dt0 f0 167 832 272 832 #arcP
Dt0 S51 .xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language lang="en">
        <name>Call ATrustSigner/TemplateManagement&#13;
GetListTemplate</name>
    </language>
</elementInfo>
' #txt
Dt0 S51 272 810 240 44 -111 -20 #rect
Dt0 S60 .xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>Print output</name>
    </language>
</elementInfo>
' #txt
Dt0 S60 608 810 112 44 -31 -8 #rect
Dt0 f7 512 832 608 832 #arcP
Dt0 f13 720 832 785 832 #arcP
Dt0 S61 .xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>Print output</name>
    </language>
</elementInfo>
' #txt
Dt0 S61 832 386 112 44 -31 -8 #rect
Dt0 f14 768 408 832 408 #arcP
Dt0 f9 944 408 1009 408 #arcP
Dt0 f21 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>Add your Signature Template to A-Trust Handy Signatur&#13;
Template should be a xml file</name>
        <nameStyle>53,5,8,0
31,5
</nameStyle>
    </language>
</elementInfo>
' #txt
Dt0 f21 128 322 432 44 -212 -18 #rect
Dt0 f22 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>Remove a Signature Template on A-Trust Handy Signatur&#13;
You should provide exactly templateID</name>
        <nameStyle>8,5,8,0
1,5,8
44,5,8,0
39,5
</nameStyle>
    </language>
</elementInfo>
' #txt
Dt0 f22 128 522 448 44 -217 -18 #rect
Dt0 f24 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>Get all your Signature Templates info on A-Trust Handy Signatur&#13;
It returns your templates name and ids</name>
        <nameStyle>7,5,8,0
1,5,0
55,5,8,0
40,5
</nameStyle>
    </language>
</elementInfo>
' #txt
Dt0 f24 128 754 496 44 -243 -18 #rect
Dt0 f25 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>API KEY IN GLOBAL VARIABLE of A-TRUST-CONNECTOR project:&#13;
- ATrust.APIKey: API Key for A-Trust Handysignatur api&#13;
- ATrust.ATrustTemplateId: The ID of the signature template to be used.&#13;
</name>
        <nameStyle>26,5,8,0
4,5,8
17,5,8,0
9,5,8
4,5
13,5,0
43,5
23,5,0
48,5
</nameStyle>
    </language>
</elementInfo>
' #txt
Dt0 f25 760 90 512 76 -248 -34 #rect
Dt0 f25 -1|-1|-65536 #nodeStyle
>Proto Dt0 .type atrust.connector.demo.Data #txt
>Proto Dt0 .processKind NORMAL #txt
>Proto Dt0 .xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <swimlaneLabel>Example to integrate in your process</swimlaneLabel>
    </language>
    <swimlaneOrientation>false</swimlaneOrientation>
    <swimlaneSize>928</swimlaneSize>
    <swimlaneColor gradient="false">-1</swimlaneColor>
    <swimlaneType>POOL</swimlaneType>
    <swimlaneSpaceBefore>32</swimlaneSpaceBefore>
</elementInfo>
' #txt
>Proto Dt0 0 0 32 24 18 0 #rect
>Proto Dt0 @|BIcon #fIcon
Bk1 f89 actionTable 'out=in;
' #txt
Bk1 f89 actionCode '//Create a file that will hold the template
in.myIvyFile = new File("Template.xml", true);

//Save result in java File of ivy File
ivy.cms.getContentObjectValue("/Templates/tempsiegel1", ivy.session.contentLocale).exportContentToFile(in.myIvyFile.getJavaFile(), null);
' #txt
Bk1 f89 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>Get TemplateSiegel from CMS&#13;
/template/templatesiegel</name>
    </language>
</elementInfo>
' #txt
Bk1 f89 272 138 192 44 -77 -16 #rect
Bk1 g0 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language lang="en">
        <name>in 1</name>
    </language>
</elementInfo>
' #txt
Bk1 g0 51 147 26 26 -9 14 #rect
Bk1 f0 77 160 272 160 #arcP
Bk1 g1 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language lang="en">
        <name>out 1</name>
    </language>
</elementInfo>
' #txt
Bk1 g1 691 147 26 26 -23 17 #rect
Bk1 f1 464 160 691 160 #arcP
>Proto Bk0 0 0 32 24 18 0 #rect
>Proto Bk0 @|BpmnScriptTaskIcon #fIcon
Bk2 f6 processCall ATrustSigner/TemplateManagement:AddTemplateATrust(File) #txt
Bk2 f6 requestActionDecl '<File templateFile> param;' #txt
Bk2 f6 requestMappingAction 'param.templateFile=in.myIvyFile;
' #txt
Bk2 f6 responseMappingAction 'out=in;
out.templateData=result.templateData;
' #txt
Bk2 f6 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>ATrustSigner/TemplateManagement</name>
    </language>
</elementInfo>
' #txt
Bk2 f6 240 138 208 44 -99 -8 #rect
Bk2 g0 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language lang="en">
        <name>in 1</name>
    </language>
</elementInfo>
' #txt
Bk2 g0 51 147 26 26 -10 14 #rect
Bk2 f0 77 160 240 160 #arcP
Bk2 g1 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language lang="en">
        <name>out 1</name>
    </language>
</elementInfo>
' #txt
Bk2 g1 611 147 26 26 0 5 #rect
Bk2 f1 448 160 611 160 #arcP
>Proto Bk1 0 0 32 24 18 0 #rect
>Proto Bk1 @|BpmnServiceTaskIcon #fIcon
Bk3 f100 actionTable 'out=in;
' #txt
Bk3 f100 actionCode '// Remove there code and replace by your template Id
in.templateId = ivy.session.getAttribute("deleteTemplateId").toNumber();' #txt
Bk3 f100 security system #txt
Bk3 f100 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>Passes a template id which you want to delete</name>
    </language>
</elementInfo>
' #txt
Bk3 f100 136 138 272 44 -126 -8 #rect
Bk3 g0 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language lang="en">
        <name>in 1</name>
    </language>
</elementInfo>
' #txt
Bk3 g0 51 147 26 26 0 5 #rect
Bk3 f0 77 160 136 160 #arcP
Bk3 g1 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language lang="en">
        <name>out 1</name>
    </language>
</elementInfo>
' #txt
Bk3 g1 467 147 26 26 0 5 #rect
Bk3 f1 408 160 467 160 #arcP
>Proto Bk2 0 0 32 24 18 0 #rect
>Proto Bk2 @|BpmnScriptTaskIcon #fIcon
Bk4 f0 processCall ATrustSigner/TemplateManagement:DeleteTemplateATrust(Number) #txt
Bk4 f0 requestActionDecl '<Number templateId> param;' #txt
Bk4 f0 requestMappingAction 'param.templateId=in.templateId;
' #txt
Bk4 f0 responseMappingAction 'out=in;
' #txt
Bk4 f0 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>ATrustSigner/TemplateManagement</name>
    </language>
</elementInfo>
' #txt
Bk4 f0 240 138 208 44 -99 -8 #rect
Bk4 g0 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language lang="en">
        <name>in 1</name>
    </language>
</elementInfo>
' #txt
Bk4 g0 51 147 26 26 -23 15 #rect
Bk4 f1 77 160 240 160 #arcP
Bk4 g1 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language lang="en">
        <name>out 1</name>
    </language>
</elementInfo>
' #txt
Bk4 g1 659 147 26 26 0 5 #rect
Bk4 f2 448 160 659 160 #arcP
>Proto Bk3 0 0 32 24 18 0 #rect
>Proto Bk3 @|BpmnServiceTaskIcon #fIcon
Bk5 f21 processCall ATrustSigner/TemplateManagement:GetListTemplate() #txt
Bk5 f21 requestActionDecl '<> param;' #txt
Bk5 f21 responseMappingAction 'out=in;
out.templateData=result.templateData;
' #txt
Bk5 f21 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>ATrustSigner/TemplateManagement</name>
    </language>
</elementInfo>
' #txt
Bk5 f21 256 138 208 44 -99 -8 #rect
Bk5 g0 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language lang="en">
        <name>in 1</name>
    </language>
</elementInfo>
' #txt
Bk5 g0 51 147 26 26 0 5 #rect
Bk5 f0 77 160 256 160 #arcP
Bk5 g1 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language lang="en">
        <name>out 1</name>
    </language>
</elementInfo>
' #txt
Bk5 g1 707 147 26 26 0 5 #rect
Bk5 f1 464 160 707 160 #arcP
>Proto Bk4 0 0 32 24 18 0 #rect
>Proto Bk4 @|BpmnServiceTaskIcon #fIcon
Bk0 g0 51 243 26 26 0 5 #rect
Bk0 g1 563 243 26 26 0 5 #rect
Bk0 f1 actionTable 'out=in;
' #txt
Bk0 f1 actionCode 'ivy.log.info("A-Trust call resultCode {0}", in.templateData.resultCode);
ivy.log.info("A-Trust call list of template {0}", in.templateData.listTemplates);' #txt
Bk0 f1 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>console log</name>
    </language>
</elementInfo>
' #txt
Bk0 f1 232 234 112 44 -32 -8 #rect
Bk0 f2 77 256 232 256 #arcP
Bk0 f0 344 256 563 256 #arcP
>Proto Bk5 0 0 32 24 18 0 #rect
>Proto Bk5 @|BpmnScriptTaskIcon #fIcon
Bk6 g0 51 243 26 26 0 5 #rect
Bk6 g1 563 243 26 26 0 5 #rect
Bk6 f1 actionTable 'out=in;
' #txt
Bk6 f1 actionCode 'ivy.log.info("A-Trust call result code {0}", in.templateData.resultCode);
ivy.log.info("A-Trust call template id {0}", in.templateData.sigTemplateId);

// prepare data for delete template
ivy.session.setAttribute("deleteTemplateId", in.templateData.sigTemplateId);' #txt
Bk6 f1 security system #txt
Bk6 f1 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>console log</name>
    </language>
</elementInfo>
' #txt
Bk6 f1 216 234 112 44 -32 -8 #rect
Bk6 f2 77 256 216 256 #arcP
Bk6 f0 328 256 563 256 #arcP
>Proto Bk6 0 0 32 24 18 0 #rect
>Proto Bk6 @|BpmnScriptTaskIcon #fIcon
Dt0 f15 mainOut f2 tail #connect
Dt0 f2 head f1 in #connect
Dt0 f1 out f5 tail #connect
Dt0 f5 head f11 mainIn #connect
Dt0 f4 mainOut f23 tail #connect
Dt0 f23 head S11 g0 #connect
Dt0 S11 g1 f3 tail #connect
Dt0 f3 head S21 g0 #connect
Dt0 f16 mainOut f6 tail #connect
Dt0 f12 head f19 mainIn #connect
Dt0 f6 head S31 g0 #connect
Dt0 S31 g1 f8 tail #connect
Dt0 f8 head S41 g0 #connect
Dt0 S41 g1 f12 tail #connect
Dt0 f17 mainOut f0 tail #connect
Dt0 f0 head S51 g0 #connect
Dt0 S51 g1 f7 tail #connect
Dt0 f7 head S60 g0 #connect
Dt0 S60 g1 f13 tail #connect
Dt0 f13 head f20 mainIn #connect
Dt0 S21 g1 f14 tail #connect
Dt0 f14 head S61 g0 #connect
Dt0 S61 g1 f9 tail #connect
Dt0 f9 head f10 mainIn #connect
Bk1 g0 m f0 tail #connect
Bk1 f0 head f89 mainIn #connect
Bk1 f1 head g1 m #connect
Bk1 f89 mainOut f1 tail #connect
Bk1 0 0 768 336 0 #ivRect
Bk2 g0 m f0 tail #connect
Bk2 f0 head f6 mainIn #connect
Bk2 f1 head g1 m #connect
Bk2 f6 mainOut f1 tail #connect
Bk2 0 0 688 328 0 #ivRect
Bk3 g0 m f0 tail #connect
Bk3 f0 head f100 mainIn #connect
Bk3 f1 head g1 m #connect
Bk3 f100 mainOut f1 tail #connect
Bk3 0 0 544 320 0 #ivRect
Bk4 g0 m f1 tail #connect
Bk4 f1 head f0 mainIn #connect
Bk4 f2 head g1 m #connect
Bk4 f0 mainOut f2 tail #connect
Bk4 0 0 736 368 0 #ivRect
Bk5 g0 m f0 tail #connect
Bk5 f0 head f21 mainIn #connect
Bk5 f1 head g1 m #connect
Bk5 f21 mainOut f1 tail #connect
Bk5 0 0 776 360 0 #ivRect
Bk0 g0 m f2 tail #connect
Bk0 f2 head f1 mainIn #connect
Bk0 f1 mainOut f0 tail #connect
Bk0 f0 head g1 m #connect
Bk0 0 0 640 512 0 #ivRect
Bk6 g0 m f2 tail #connect
Bk6 f2 head f1 mainIn #connect
Bk6 f1 mainOut f0 tail #connect
Bk6 f0 head g1 m #connect
Bk6 0 0 640 512 0 #ivRect
